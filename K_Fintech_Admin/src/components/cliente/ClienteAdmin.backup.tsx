...
const styles = {
  ...
  tableHeader: {
    ...
    fontWeight: '600',
    color: '#1e293b',  // Texto oscuro para mejor visibilidad
    padding: '12px',
...
const ClienteAdmin: React.FC = () => {
  ...
  const fetchClientes = async () => {
    try {
      ...
      const clientesTransformados = data.map((item: any) => ({
        ...
        tipoCliente: 'NATURAL', // Valor por defecto
        identificacion: item.cedula_cliente || item.identificacion || '',
        nombreRazonSocial: item.nombre_cliente || item.nombreRazonSocial || '',
        direccion: item.direccion_cliente || item.direccion || '',
        telefono: item.celular_cliente || item.telefono || '',
        email: item.correo_cliente || item.email || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validarIdentificacion(formData.identificacion, formData.tipoCliente)) {
  ...
  const validarIdentificacion = (identificacion: string, tipoCliente: string): boolean => {
    if (tipoCliente === 'NATURAL') {
      // Validar cédula ecuatoriana (10 dígitos)
      if (!/^\d{10}$/.test(identificacion)) return false;
      return validarDigitoVerificador(identificacion);
    } else {
      // Validar RUC (13 dígitos, termina en 001)
      if (!/^\d{13}$/.test(identificacion)) return false;
      return identificacion.endsWith('001');
    }
  };

  const validarDigitoVerificador = (cedula: string): boolean => {
        ...
        <div style={styles.formSection}>
          ...
          <form onSubmit={handleSubmit} style={styles.form}>
            <div style={styles.formRow}>
              ...
              <div style={styles.formGroup}>
                <label style={styles.label}>{formData.tipoCliente === 'NATURAL' ? 'Cédula' : 'RUC'}:</label>
                <input
                  type="text"
                  name="identificacion"
                  value={formData.identificacion}
                  onChange={handleInputChange}
                  placeholder={formData.tipoCliente === 'NATURAL' ? '10 dígitos' : '13 dígitos terminados en 001'}
                  style={styles.input}
                  required
                />
              </div>
